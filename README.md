...
class Ivan:
    ...
    def __init__(self, name):
        if name == 'Ivan':
            self.name = f"Yes, i am {name}"
        else:
            self.name = f"No, i am not {name}"
...
class MyClass:
    ...
    def get_value(self):
        # Проверка на существование атрибута перед его возвратом
        if hasattr(self, '_value'):
            return self._value
        else:
            return "Атрибут _value был удален"

    def del_value(self):
        del self._value

    value = property(get_value, set_value, del_value, "Property value")
...
![Меню](https://github.com/Tureckij/Software_Engineering/blob/Тема_9/9_module/S1.png)

### Выводы

Создали классы Tomato, TomatoBush и Gardener, реализовав структуру для моделирования процесса выращивания и сбора урожая томатов. Класс Tomato определяет стадии созревания помидора, TomatoBush управляет списком помидоров и их созреванием, а Gardener ухаживает за растением и собирает урожай. В коде также выполнены тесты: вызов справки, уход за растением и сбор урожая после его созревания.
  

## Общие выводы по теме
Углубились в изучении ООП на питоне. Более подробно познакомились с полиморфизмом, инкапсуляцией и наследованием.
